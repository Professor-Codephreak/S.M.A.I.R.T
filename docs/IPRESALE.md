...
Unauthorized()
```
Description: Triggered when an address without the required permissions attempts an action reserved for authorized users.
...
```
Description: Emitted when an operation is attempted in a state where it is not allowed.
Use Case: Ensures that operations are performed only in valid states (e.g., finalizing a presale that hasn't ended).
```solidity
SoftCapNotReached()
```
Description: Raised when an attempt to finalize the presale is made, but the soft cap has not been met.
Use Case: Protects participants by ensuring that a presale cannot be finalized without meeting its minimum funding goal.
```solidity

HardCapExceed()
```
Description: Emitted when a purchase exceeds the presale's maximum allowable contributions (hard cap).
Use Case: Prevents the contract from accepting more funds than it is designed to handle.
```solidity
NotClaimable()
```
Description: Triggered when an attempt is made to claim tokens by a user who hasn't contributed or when claiming is not yet allowed.
Use Case: Ensures that only eligible participants can claim tokens.
```solidity
NotInPurchasePeriod()
```
Description: Raised when a purchase or refund attempt is made outside the designated presale period.
Use Case: Maintains the integrity of the presale timeline.
```solidity
PurchaseBelowMinimum()
```
Description: Emitted when a purchase amount is less than the minimum allowed contribution.
Use Case: Enforces minimum contribution requirements to participate in the presale.
```solidity
PurchaseLimitExceed()
```

Description: Raised when a participant attempts to contribute more than the maximum allowed per individual.
Use Case: Prevents any single participant from dominating the presale.
```solidity
NotRefundable()
```
Description: Triggered when a refund is requested under conditions that do not allow refunds.
Use Case: Ensures that refunds are only processed under valid circumstances, such as when the presale is canceled.
```solidity
LiquificationFailed()
```
Description: Raised when an attempt to add liquidity to a pool fails.
...
```
Description: Emitted when the initialization parameters provided to the contract are invalid.
Use Case: Ensures the contract is initialized with valid and correct parameters.
```solidity
InvalidCapValue()
```
Description: Triggered when the provided cap values (soft cap or hard cap) are invalid.
Use Case: Prevents the presale from operating with incorrect funding limits.
```solidity
InvalidLimitValue()
```

Description: Raised when participant limits (minimum or maximum contributions) are invalid.
Use Case: Ensures participant limits are set correctly.
```solidity
InvalidLiquidityValue()
```
Description: Emitted when the liquidity parameters provided to the contract are invalid.
Use Case: Ensures proper configuration for liquidity provisioning.
```solidity
InvalidTimestampValue()
```
Description: Triggered when the provided timestamps (e.g., start and end times) are invalid.
...
```
Description: Emitted when the presale contract owner deposits tokens to be sold during the presale.
Parameters:
...
```
Description: Emitted when a participant contributes ETH to the presale.
Parameters:
...
```
Description: Emitted when the presale is successfully finalized.
Parameters:
...
```
Description: Emitted when a participant successfully claims a refund.
Parameters:
...
```
Description: Emitted when participants claim their purchased tokens after the presale is finalized.
Parameters:
...
```
Description: Emitted when the presale is canceled by the contract owner.
Parameters:
...
```
Description: Allows the presale contract owner to deposit tokens into the contract for sale.
...
```
Description: Finalizes the presale, allowing token distribution and withdrawal of funds.
...
```
Description: Cancels the presale, enabling participants to claim refunds.
Returns: A boolean indicating success.
...
```
Description: Allows participants to claim their purchased tokens after the presale is finalized.
...
```
Description: Enables participants to request a refund if the presale is canceled or under eligible conditions.
